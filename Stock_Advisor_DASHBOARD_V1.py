...
# --- 1. V7 다중 분류 마스터 AI 모델 구조 (14 Features, 3 Class) ---
...
def analyze_with_llama3(ticker_name, custom_event): # 👈 news_text 매개변수 삭제
    try:
        ...
        [분석 지침]
        1. **섹터 동조화(Spillover) 분석**: 금리 인하 등 거대한 글로벌 매크로 이벤트가 해당 종목이 속한 테마의 투자 심리(Multiple Re-rating)나 글로벌 공급망에 미치는 낙수효과를 최우선으로 고려하십시오.
        2. **직접/간접 영향 분석**: 이벤트가 기업의 매출, 원가에 주는 직접적 타격뿐만 아니라 거시 경제적 연결 고리(Transmission Mechanism)를 구체적으로 설명하십시오.
        3. **노이즈 판단**: 글로벌 이벤트와 해당 기업의 비즈니스가 완전히 무관한 테마라면 억지로 엮지 말고 단호하게 0점을 부여하십시오.
...
def load_ensemble_models(gru_path, lgb_path):
    ...
    try:
        if not os.path.exists(gru_path):
            print(f"❌ [경로 에러] GRU 모델 파일을 찾을 수 없습니다: {gru_path}")
            return None, None, device
        if not os.path.exists(lgb_path):
            print(f"❌ [경로 에러] LGBM 모델 파일을 찾을 수 없습니다: {lgb_path}")
            return None, None, device
        ...
        print("✅ [성공] V7 AI 모델 로드 완료!")
        return model_gru, model_lgb, device
        
    except Exception as e: 
        # 🌟 규격 충돌이나 파일 깨짐 등 진짜 에러 이유를 터미널에 강제 출력!
        print(f"🔥 [치명적 에러] 모델 로드 중 예외 발생: {e}")
        return None, None, device
...
# --- 5. 메인 실행부 ---
# 🌟 로컬 디버깅 핵심: 현재 실행 중인 파이썬 파일의 폴더 위치를 절대 경로로 잡기
...
RESULT_CSV = os.path.join(BASE_DIR, "morning_scan_result.csv")
SECTOR_UP_CSV = os.path.join(BASE_DIR, "sector_upjong.csv")
SECTOR_TH_CSV = os.path.join(BASE_DIR, "sector_theme.csv")
ETF_CSV = os.path.join(BASE_DIR, "etf_scanner_result.csv")
            ...
            with st.spinner(f"[{name}] 데이터 분석 중..."):
                try:
                    ...
                    if not feats_df.empty and len(feats_df) >= 60:
                        ...
                        # 🌟 [수정 완료] 3-Class V7 모델용 확률 추출 로직 (파이 차트 NameError 방지)
                        with torch.no_grad(): 
                            gru_probs = torch.softmax(model_gru(inp), dim=1).cpu().numpy()[0]
                        ...
                        base_prob_pct = prob_buy # 스캐너 메인 점수는 '강력매수(Class 2)' 확률을 사용합니다.
